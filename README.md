...
This is a side-view cross-section showing the axial Y direction (horizontal) and the
transverse X direction (vertical, signed). The injector sits at Y = 99.5 mm, X = 0
(right edge, centreline — cyan triangle), injecting leftward (−Y direction) with a 10°
half-cone angle that opens symmetrically in ±X. Droplets are coloured by diameter; gas
temperature is shown in the background — at this early time the gas is still at ~800 K
and combustion has not yet started.
